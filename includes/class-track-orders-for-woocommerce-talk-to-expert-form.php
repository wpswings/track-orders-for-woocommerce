 ...
 */
class Track_Orders_For_Woocommerce_Talk_To_Expert_Form {
	 ...
	 * @var string
	 */
	private static $wps_pgfw_plugin_name_label = 'Track Orders For WooCommerce';
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_wps_tofw_submit_talk_to_expert', array( $this, 'wps_pgfw_submit_talk_to_expert' ) );
	}
	...
	private function wps_pgfw_is_plugin_dashboard_screen() {
		...
		return 'track_orders_for_woocommerce_menu' === $page;
	}
			...
			<div class="pgfw-expert-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="pgfw-expert-modal-title">
				<div class="pgfw-expert-modal__header">
					<div class="pgfw-expert-modal__copy">
						...
						<h2 id="pgfw-expert-modal-title"><?php esc_html_e( 'Talk to an Expert', 'track-orders-for-woocommerce' ); ?></h2>
						<p><?php esc_html_e( 'Share your store goals and our team will reach out with the right next step.', 'track-orders-for-woocommerce' ); ?></p>
					</div>
					<button type="button" class="pgfw-expert-modal__close" data-pgfw-close-expert-modal="true" aria-label="<?php esc_attr_e( 'Close expert form', 'track-orders-for-woocommerce' ); ?>">
				...
				<div class="pgfw-expert-modal__body">
					<form class="pgfw-expert-form" data-pgfw-expert-form="true" data-pgfw-expert-form-panel="true" novalidate>
						...
						<div class="pgfw-expert-form__actions">
							<button
								...
								data-pgfw-submit-label="<?php echo esc_attr__( 'Submit Request', 'track-orders-for-woocommerce' ); ?>"
								data-pgfw-submit-loading-label="<?php echo esc_attr__( 'Sending...', 'track-orders-for-woocommerce' ); ?>"
							>
								<?php esc_html_e( 'Submit Request', 'track-orders-for-woocommerce' ); ?>
							</button>
						</div>
					...
					<div class="pgfw-expert-thank-you" data-pgfw-expert-thank-you="true" aria-hidden="true" hidden>
						<div class="pgfw-thank-you-card pgfw-thank-you-card--modal" aria-live="polite">
							...
							<p class="pgfw-thank-you-card__eyebrow"><?php esc_html_e( 'Talk to an Expert', 'track-orders-for-woocommerce' ); ?></p>
							<h2 class="pgfw-thank-you-card__title"><?php esc_html_e( 'Thank You', 'track-orders-for-woocommerce' ); ?></h2>
							<p class="pgfw-thank-you-card__message" data-pgfw-expert-thank-you-message="true"><?php esc_html_e( 'Thank you for submitting your request.', 'track-orders-for-woocommerce' ); ?></p>
							<p class="pgfw-thank-you-card__meta"><?php esc_html_e( 'Our team will review the details and contact you with the right next step. Redirecting you back to the dashboard in a moment.', 'track-orders-for-woocommerce' ); ?></p>
							<p class="pgfw-thank-you-card__action">
								<button type="button" class="pgfw-rail-action pgfw-rail-action--dark" data-pgfw-close-expert-modal="true"><?php esc_html_e( 'Done', 'track-orders-for-woocommerce' ); ?></button>
							</p>
						</div>
					</div>
				</div>
	...
	private function wps_pgfw_get_custom_form_fields() {
		return array(
			'firstname' => array(
				'label'       => esc_html__( 'First Name', 'track-orders-for-woocommerce' ),
				'type'        => 'text',
				'placeholder' => esc_html__( 'John', 'track-orders-for-woocommerce' ),
				'required'    => false,
			),
			'lastname'  => array(
				'label'       => esc_html__( 'Last Name', 'track-orders-for-woocommerce' ),
				'type'        => 'text',
				'placeholder' => esc_html__( 'Doe', 'track-orders-for-woocommerce' ),
				'required'    => false,
			),
			'email'      => array(
				'label'       => esc_html__( 'Work Email', 'track-orders-for-woocommerce' ),
				'type'        => 'email',
				'placeholder' => esc_html__( 'name@company.com', 'track-orders-for-woocommerce' ),
				'required'    => true,
				'full_width'  => true,
			),
			'phone'      => array(
				'label'       => esc_html__( 'Contact Number', 'track-orders-for-woocommerce' ),
				'type'        => 'text',
				'placeholder' => esc_html__( '+1 000 000 0000', 'track-orders-for-woocommerce' ),
				'required'    => false,
			),
			'what_services_do_you_need_help_with' => array(
				'label'       => esc_html__( 'What services do you need help with?', 'track-orders-for-woocommerce' ),
				'type'        => 'checkbox_group',
			...
			'budget'      => array(
				'label'       => esc_html__( 'Budget', 'track-orders-for-woocommerce' ),
				'type'        => 'select',
			...
			'message'    => array(
				'label'       => esc_html__( 'What do you need help with?', 'track-orders-for-woocommerce' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Share your goals, blockers, or the service you need.', 'track-orders-for-woocommerce' ),
				'required'    => false,
				'full_width'  => true,
			),
		);
	...
	private function wps_pgfw_get_talk_to_expert_service_options() {
		return array(
			'seo_services'                     => esc_html__( 'SEO services', 'track-orders-for-woocommerce' ),
			'google_ads_setup_and_ga4_setup'  => esc_html__( 'Google Ads Setup and GA4 setup', 'track-orders-for-woocommerce' ),
			'speed_optimization'              => esc_html__( 'Speed Optimization', 'track-orders-for-woocommerce' ),
			'woocommerce_development_services' => esc_html__( 'WooCommerce Development Services', 'track-orders-for-woocommerce' ),
		);
	}
	...
	public function wps_pgfw_submit_talk_to_expert() {
		check_ajax_referer( 'wps_tofw_talk_to_expert_nonce', 'nonce' );

		if ( ! current_user_can( 'manage_options' ) && ! current_user_can( 'manage_woocommerce' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'You are not allowed to submit this request.', 'track-orders-for-woocommerce' ),
				),
				403
			);
		}
		...
		if ( empty( $form_data ) || ! is_array( $form_data ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'The request could not be processed. Please try again.', 'track-orders-for-woocommerce' ),
				),
				400
			);
		}
		...
		if ( empty( $sanitized_data['email'] ) || ! is_email( $sanitized_data['email'] ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Please enter a valid email address.', 'track-orders-for-woocommerce' ),
				),
				422
			);
		}
	...
	private function wps_pgfw_get_talk_to_expert_dashboard_url() {
		return admin_url( 'admin.php?page=track_orders_for_woocommerce_menu' );
	}
	...
	private function wps_pgfw_submit_hubspot_form( $fields ) {
		...
		if ( is_wp_error( $response ) ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'The request could not be sent right now. Please try again shortly.', 'track-orders-for-woocommerce' ),
			);
		}
	...
	private function wps_pgfw_get_success_message( $response ) {
		...
		return esc_html__( 'Thank you for submitting your request. Our team will contact you soon.', 'track-orders-for-woocommerce' );
	}
	...
	private function wps_pgfw_get_error_message( $response ) {
		...
		return esc_html__( 'Something went wrong while submitting the form. Please try again.', 'track-orders-for-woocommerce' );
	}
