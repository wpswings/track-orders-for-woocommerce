...
<div class="wps-tofw-order-tracking-section  <?php echo esc_attr( $wps_main_wrapper_class ); ?>">
	...
	if ( true == $allowed ) {
		...
		<section class="wps-order-section about-section details-section <?php echo esc_html( $wps_child_wrapper_class ); ?>">
			...
			<div class="wps_tofw_order_tab-temp-alpha wps-order-details-header">
			...
			<table class="wps_tofw_shop_table order_details wps-product-details-table wps-tofw-track-order-table">
				<thead>
					<tr>
						...
						<?php if ( '' != $expected_delivery_date || '' != $expected_delivery_time || '' != $order_delivered_date || ! empty( $wps_status_change_time['wc-completed'] ) ) { ?>
...
'></span>
						...
						( isset( $wps_status_change_time['wc-completed'] ) && ! empty( $wps_status_change_time['wc-completed'] ) )
						) {
							?>
								...
								if ( 'on' != $wps_tofw_enable_track_order_popup ) {
									...
									?>
										...
										<span>
											...
											} else if ( '' == $order_delivered_date && 'wc-cancelled' != $order_status && 'wc-completed' == $order_status && empty( $expected_delivery_time ) && empty( $expected_delivery_date ) ) {
												esc_html_e( 'Delivered', 'track-orders-for-woocommerce' );
												echo esc_html__( ' on ', 'track-orders-for-woocommerce' ) . esc_html( date_i18n( 'd F, Y H:i', strtotime( $wps_status_change_time['wc-completed'] ) ) );
											}
											?>
											</span>
										</span>
